...
| Legacy HMAC (modo actual) | `POST /api/sync-hmac/batch/` | headers `X-Device-*` + `commands[]` | HMAC request-level + nonce | `apps.modulos.sync` (legacy) | contrato legacy (`status=OK/ERROR`) |
| Legacy HMAC (modo wrapper) | `POST /api/sync-hmac/batch/` | igual que legacy | HMAC legacy + traducción a core | `sync_engine` (wrapper interno) | contrato legacy + headers de deprecación |
...
- `SYNC_HMAC_WRAPPER_ENABLED=false`  
  Mantiene `sync-hmac` en motor legacy por defecto.
- `SYNC_V2_MAX_SKEW_SECONDS=300`  
  Ventana de reloj para request-level en v2.
- `SYNC_LEGACY_HMAC_SUNSET=2026-03-31T00:00:00Z`  
  Header `Sunset` cuando `sync-hmac` opera en modo wrapper.
...
- `sync-hmac` sigue estable por defecto; modo wrapper activable por flag.
