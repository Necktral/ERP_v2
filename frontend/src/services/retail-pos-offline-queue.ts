...
import type { CheckoutPosTicketPayload, OpenPosTicketPayload } from 'src/services/retail-pos.service';
...
export type PosOfflineCreateCheckoutPayload = {
  open_ticket: OpenPosTicketPayload;
  checkout: CheckoutPosTicketPayload;
};
...
function nextBackoffMs(attempt: number): number {
  ...
  return Math.min(base, POS_OFFLINE_BACKOFF_CAP_MS);
}
...
function sanitizeStatus(value: string): PosOfflineCommandStatus {
  if (value === 'PENDING' || value === 'PROCESSING' || value === 'FAILED' || value === 'DONE') return value;
  return 'PENDING';
}

function sanitizeKind(value: string): PosOfflineCommandKind {
...
function normalizeCommand(raw: unknown): PosOfflineCommand | null {
  ...
  return {
    ...
    kind: sanitizeKind(safeString(row.kind, 'CREATE_AND_CHECKOUT')),
...
function isRetryableError(error: unknown): { retryable: boolean; message: string } {
  if (typeof error === 'object' && error !== null) {
    ...
    const msg = apiMsg || maybe.message || safeString((error as { code?: string }).code, 'Unknown error');
...
function isDue(command: PosOfflineCommand, nowMs: number): boolean {
  ...
  if (command.status === 'PROCESSING') return false;
...
export function enqueuePosOfflineCommand(input: {
  ...
  if (!dedupeKey) throw new Error('dedupe_key es requerido para cola offline POS');
  ...
  if (!result) throw new Error('No fue posible registrar comando offline POS');
...
export async function drainPosOfflineQueue(options: PosOfflineDrainOptions): Promise<PosOfflineDrainResult> {
  ...
  for (const row of due) {
    attempted += 1;
    updateQueue((state) => {
      const commands: PosOfflineCommand[] = state.commands.map((cmd): PosOfflineCommand =>
        cmd.id === row.id
          ? {
              ...cmd,
